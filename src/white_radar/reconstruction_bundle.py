...
def render_reconstruction_report(reconstruction: AttackReconstruction) -> str:
    ...
    all_transfers = [
        transfer for case in reconstruction.transactions for transfer in case.transfers
    ]
    committed_transfers = sum(item.execution_effect == "committed" for item in all_transfers)
    attempted_transfers = sum(
        item.execution_effect == "attempted_reverted" for item in all_transfers
    )
    unknown_transfers = len(all_transfers) - committed_transfers - attempted_transfers
    state_accounts = sum(
    ...
    flow_edges = [
        item
        for item in reconstruction.edges
        if item.asset_type is not None and item.execution_effect == "committed"
    ]
    attempted_flow_edges = [
        item
        for item in reconstruction.edges
        if item.asset_type is not None and item.execution_effect == "attempted_reverted"
    ]
    core_contexts = [item for item in contexts if item.core_candidate]
    lines = [
        f"# WhiteRadar Incident reconstruction {reconstruction.reconstruction_id}",
        "",
        (
            f"Generated by **{_plain(reconstruction.tool_name)} "
            f"{_plain(reconstruction.tool_version)}** at `{reconstruction.generated_at}`."
        ),
        "",
        "## Executive summary",
        "",
        (
            ...
            f"entities**, **{calls} call frames**, **{events} emitted events**, "
            f"**{committed_transfers} committed asset transfers**, and "
            f"**{attempted_transfers} reverted transfer attempts** "
        ...
        f"- Frontier addresses discovered: `{coverage.frontier_addresses_discovered}`",
        ...
        (
            "- Candidate transactions not reconstructed: "
            f"`{coverage.candidate_transactions_not_reconstructed}`"
        ),
        f"- Transaction reconstruction failures: `{coverage.transaction_failures}`",
        (
            f"- Reconstructed block span: `{coverage.reconstructed_start_block}` through "
            f"`{coverage.reconstructed_end_block}`"
        ),
        f"- Core candidate transactions: `{coverage.core_transactions}`",
        f"- High-fanout hub addresses detected: `{len(coverage.hub_addresses_detected)}`",
        (
            "- Hub history records suppressed from expansion: "
            f"`{coverage.hub_history_records_suppressed}`"
        ),
        f"- Trace coverage: `{traced}/{len(contexts)}` transactions",
        f"- Committed asset transfers: `{committed_transfers}`",
        f"- Reverted transfer attempts: `{attempted_transfers}`",
        f"- Asset-transfer effects with unknown finality: `{unknown_transfers}`",
        ...
        f"- Hub detection minimum records: `{reconstruction.limits.hub_min_records}`",
        (
            "- Hub detection minimum counterparties: "
            f"`{reconstruction.limits.hub_min_counterparties}`"
        ),
        (
            "- Maximum retained candidate transactions per hub: "
            f"`{reconstruction.limits.max_hub_candidate_transactions}`"
        ),
        "",
        ...
        "",
        "## High-signal core candidate path",
        "",
        (
            "This compact path contains the operator-selected seed and transactions discovered "
            "within one direct evidence hop. It is intended for analyst triage; it is not an "
            "automatic attribution or a claim that every row is malicious."
        ),
        "",
        "| Phase | Block | Transaction | From | To | Function / selector | Status | Score | Core basis |",
        "|---|---:|---|---|---|---|---|---:|---|",
    ]
    for item in core_contexts:
        ...
        lines.append(
            "| {} | {} | {} | `{}` | `{}` | `{}` | {} | {} | {} |".format(
                ...
                "; ".join(_plain(value) for value in item.core_reasons),
            )
        )
    if not core_contexts:
        lines.append("|  |  |  |  |  |  |  |  | No core candidate path reconstructed |")
    lines.extend(
        [
            "",
            "The complete compact export is available in `core_path.csv`.",
            "",
            "## Complete chronological candidate inventory",
            "",
            "| Phase | Block | Tx index | Transaction | From | To | Function / selector | Status | Hop | Score | Why included |",
            "|---|---:|---:|---|---|---|---|---|---:|---:|---|",
        ]
    )
    ...
    if flow_edges:
        ...
        lines.append("|  |  |  |  |  |  |  | No committed standard asset flow reconstructed |")

    lines.extend(
        [
            "",
            "## Reverted transfer attempts",
            "",
            (
                "These trace frames are execution evidence but did not change final chain state. "
                "They are excluded from the committed asset ledger and follow-funds frontier."
            ),
            "",
            "| Block | Transaction | Type | Asset | From | To | Amount | Evidence |",
            "|---:|---|---|---|---|---|---:|---|",
    ...
    if attempted_flow_edges:
        for edge in attempted_flow_edges[:200]:
            lines.append(
                "| {} | [{}]({}/tx/{}) | {} | `{}` | `{}` | `{}` | `{}` | `{}` |".format(
                    edge.block_number,
                    _short(edge.transaction_hash),
        ...
        lines.append("|  |  |  |  |  |  |  | No reverted transfer attempts reconstructed |")
    ...
    if event_counts:
        for identity, count in sorted(event_counts.items(), key=lambda item: (-item[1], item[0])):
            lines.append(
    ...
    if state_accounts or storage_changes:
        ...
        lines.append(
            "- State-diff coverage is unavailable: no configured RPC provider returned "
            "compatible `prestateTracer` diff-mode evidence. Empty state CSV files therefore "
            "mean 'not observed from this provider', not 'no state changed'."
        )
    ...
    lines.extend(
        [
            ...
            "- `core_path.csv`: compact seed plus direct one-hop candidate path for analyst triage.",
            ...
            "- `transfers.csv`: raw and normalized native/token transfer evidence with committed/reverted/unknown effect classification.",
            ...
            "",
            f"Generated by {reconstruction.tool_name} {reconstruction.tool_version}.",
        ]
    )
    return "\n".join(lines) + "\n"
        ...
        nodes.append(
            {
                ...
                "core_reasons": list(context.core_reasons),
            }
        )
    return nodes
            ...
            edges.append(
                {
                    ...
                    "execution_effect": transaction_effect,
                }
            )
        ...
        edges.append(
            {
                ...
                "execution_effect": edge.execution_effect,
            }
        )
    return edges
...
def _graphml(reconstruction: AttackReconstruction) -> str:
    ...
    for edge in _graph_edges(reconstruction):
        edges.append(
            ...
            '<data key="transaction">{}</data><data key="execution_effect">{}</data></edge>'.format(
                ...
                html.escape(str(edge.get("execution_effect") or "unknown")),
            )
        )
    return (
...
<head>
  ...
  <title>WhiteRadar Incident reconstruction graph</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .node.core { stroke: #76f7c5; stroke-width: 2.5; }
...
<body>
<header>
  <h1>WhiteRadar Incident reconstruction graph</h1>
  <div id="meta"></div>
  <div id="toolbar">
...
const nodeElements = nodes.map(node=>{const circle=document.createElementNS(ns,'circle');circle.setAttribute('r',node.kind==='transaction'?10:node.roles.includes('transaction_target')?9:7);circle.classList.add('node',node.kind||'unknown');if(node.core_candidate)circle.classList.add('core');if(node.id===`tx:${data.seed}`)circle.classList.add('seed');circle.addEventListener('click',()=>show(node,'Node'));nodeLayer.appendChild(circle);return circle;});
...
def write_reconstruction_bundle(
    ...
    *,
    ...
    transaction_rows = [
        {
            **item.to_dict(),
            ...
            "core_reasons": ";".join(item.core_reasons),
            ...
            event_rows.append(
                {
                    ...
                    "hop": context.hop,
                    **event.to_dict(),
                    "arguments": json.dumps(event.arguments, sort_keys=True, separators=(",", ":")),
    ...
    artifacts: dict[str, str] = {
        ...
        "core_path.csv": _csv_text(
            transaction_fields,
            [row for row in transaction_rows if row["core_candidate"]],
        ),
