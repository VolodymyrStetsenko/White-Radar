...
def render_incident_report(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "> Status: unverified security signal. Priority is not proof of malicious intent.",
        "",
        "## Executive summary",
        "",
    ...
    lines.extend(
        [
            "",
            "## Authorization and disclosure checklist",
            "",
            "- [ ] Re-confirm the exact published or contracted scope.",
            "- [ ] Validate the signal independently against the chain explorer.",
            "- [ ] Preserve only the minimum evidence required for triage.",
            "- [ ] Use the protocol's official private security channel.",
            "- [ ] Do not replay, replace, front-run, or broadcast a competing transaction.",
            "- [ ] Do not publish an unpatched critical issue.",
            "",
            "Generated by White Radar. Human validation is required.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def render_digest(events: list[RadarEvent], chains: dict[str, ChainConfig], *, hours: int) -> str:
    ...
    lines.extend(["", "Priority is triage, not proof of malicious intent."])
    return "\n".join(lines)[:4000]
